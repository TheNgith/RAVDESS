...
def add_noise(y, noise_scale=(0.01, 0.05)):
    if y.size == 0:
        return y
    scale = np.random.uniform(*noise_scale)
    noise = np.random.randn(len(y)) * scale
    return y + noise

def random_gain(y, gain_range=(-6, 6)):
    # gain_range in dB
    if y.size == 0:
        return y
    db = np.random.uniform(*gain_range)
    factor = 10 ** (db / 20)
    return y * factor

def random_time_stretch(y, low=0.9, high=1.1):
    if y.size == 0:
        return y
    rate = np.random.uniform(low, high)
    return librosa.effects.time_stretch(y, rate=rate)

def random_pitch_shift(y, sr, semitones=(-2, 2)):
    if y.size == 0:
        return y
    steps = np.random.uniform(*semitones)
    if abs(steps) < 0.1:
        return y
...
def augment_waveform(y, sr):
    ...
    if np.random.rand() < 0.3:
        y = random_time_stretch(y, 0.9, 1.1)
    if np.random.rand() < 0.3:
        y = random_pitch_shift(y, sr, (-2, 2))
    return y

def pad_or_crop(y, sr, duration, train_mode=False):
    target_len = int(sr * duration)
    if len(y) < target_len:
        pad = target_len - len(y)
        left = pad // 2
